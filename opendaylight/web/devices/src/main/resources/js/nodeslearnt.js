    ...
    modal : {
      ...
      body : {
        updateNode : function(nodeId, switchDetails, tiers) {
          ...
          var $select = one.lib.form.select.create([ "Allow reactive forwarding", "Proactive forwarding only" ]);
      ...
      footer : {
        updateNode : function() {
          var footer = [];
          var saveButton = one.lib.dashlet.button.single("Save", one.f.switchmanager.nodesLearnt.id.modal.save,
              "btn-primary", "");
            ...
            formatter : function(items) {
              $.each(items, function(index, item) {
                ...
                item.nodeName = '<a href="#" id="' + item.nodeId + '" switchDetails='
                    + encodeURIComponent(JSON.stringify(item)) + ' privilege=' + data.privilege + '>' + nodeNameEntry
                    + '</a>';
                ...
                item.ports = '<span class="nodePorts" style="cursor:pointer;color: #08c" ports='
                    + encodeURIComponent(JSON.stringify(item.ports)) + ' nodeId="' + item.nodeId + '" nodeName="'
                    + nodeName + '">' + portsLength + '</span>';
              });
            },
            delay : 0
          });
          ...
          var source = new StaticDataSource({
            ...
            formatter : function(items) {
              $.each(items, function(index, item) {
                ...
                item.ports = '<span class="nodePorts" style="cursor: pointer;color: #08c" ports='
                    + encodeURIComponent(JSON.stringify(item.ports)) + ' nodeId="' + item.nodeId + '" nodeName="'
                    + item.nodeName + '">' + portsLength + '</span>';
              });
            },
            delay : 0
          });
