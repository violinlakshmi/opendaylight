...
require(['datasource', 'underscore', 'fuelux/loader.min'], function(StaticDataSource) {
        ...
        modal : {
          ...
          initialize : {
            updateNode : function(evt) {
              one.f.switchmanager.nodesLearnt.ajax.main(one.f.switchmanager.rootUrl + "/tiers",
                  function(tiers) {

                    var nodeId = decodeURIComponent(evt.target.id);
                    var h3;
                    var footer = [];
                    var $body = one.f.switchmanager.nodesLearnt.modal.body
                        .updateNode(nodeId, JSON.parse(decodeURIComponent(evt.target
                            .getAttribute("switchDetails"))), tiers);
                    ...
                    var $modal = one.lib.modal.spawn(
                        one.f.switchmanager.nodesLearnt.id.modal.configure, h3, "", footer);
                    // bind save button
                    $('#' + one.f.switchmanager.nodesLearnt.id.modal.save, $modal).click(
                        function() {
                ...
                one.f.switchmanager.nodesLearnt.ajax.main(url, function(content) {
                  ...
                  $("#" + one.f.switchmanager.nodesLearnt.id.modal.datagrid).datagrid({
                    ...
                    stretchHeight : false
                      ...
                      "loaded",
                      function() {
                        $("#" + one.f.switchmanager.nodesLearnt.id.modal.datagrid).find(
          ...
          save : function($modal) {
            var result = {};
            result['nodeName'] = $('#' + one.f.switchmanager.nodesLearnt.id.modal.form.nodeName,
                $modal).val();
            ...
            result['nodeId'] = $('#' + one.f.switchmanager.nodesLearnt.id.modal.form.nodeId, $modal)
                .val();
            result['tier'] = $('#' + one.f.switchmanager.nodesLearnt.id.modal.form.tier, $modal)
                .val();
          ...
          ajax : function(requestData, callback) {
            $.getJSON(one.f.switchmanager.rootUrl + "/nodesLearnt/update", requestData, function(
                response) {
              callback(response);
            });
          },
                      ...
                      $
                          .each(
                              ...
                              function(index, item) {
                                ...
                                var nodeNameEntry = item.nodeName ? item.nodeName
                                    : "Click to update";
                                item.nodeName = '<a href="#" id="' + item.nodeId
                                    + '" switchDetails=' + encodeURIComponent(JSON.stringify(item))
                                ...
                                item.ports = '<span class="nodePorts" style="cursor:pointer;color: #08c" ports='
                                    + encodeURIComponent(JSON.stringify(item.ports))
                                    + ' nodeId="'
                                    + item.nodeId
                                    + '" nodeName="'
                                    + nodeName
          ...
          one.f.switchmanager.subnetGatewayConfig.ajax.main(url, {}, function(content) {
            ...
            if (content.privilege === 'WRITE') {
              ...
              $button.click(function() {
                ...
                one.f.switchmanager.subnetGatewayConfig.modal.removeMultiple
                    .dialog(gatewaysToDelete)
              });
              $dashlet.append($button);
        ...
        modal : {
          initialize : {
            gateway : function() {
              ...
              var $modal = one.lib.modal.spawn(
                  one.f.switchmanager.subnetGatewayConfig.id.modal.modal, h3, "", footer);
              // bind save button
              $('#' + one.f.switchmanager.subnetGatewayConfig.id.modal.save, $modal).click(
                  function() {
            ...
            ports : function() {
              ...
              var footer = one.f.switchmanager.subnetGatewayConfig.modal.footer();
              var $modal = one.lib.modal.spawn(
                  one.f.switchmanager.subnetGatewayConfig.id.modal.ports, h3, "", footer);
              // bind save button
              $('#' + one.f.switchmanager.subnetGatewayConfig.id.modal.save, $modal).click(
                  function() {
              ...
              one.f.switchmanager.spanPortConfig.modal.ajax.nodes(function(nodes, nodeports) {
                var $body = one.f.switchmanager.subnetGatewayConfig.modal.body.ports(nodes,
                    nodeports);
                one.lib.modal.inject.body($modal, $body);
              });
              return $modal;
          ...
          save : {
            gateway : function($modal) {
              ...
              result['gatewayIPAddress'] = $(
                  '#' + one.f.switchmanager.subnetGatewayConfig.id.modal.form.gatewayIPAddress,
                  $modal).val();
            ...
            ports : function($modal) {
              ...
              result['nodeId'] = $(
                  '#' + one.f.switchmanager.subnetGatewayConfig.id.modal.form.nodeId, $modal).val();
              result['ports'] = $(
                  '#' + one.f.switchmanager.subnetGatewayConfig.id.modal.form.ports, $modal).val();
          ...
          ajax : {
            gateway : function(requestData, callback) {
              $.getJSON(one.f.switchmanager.rootUrl + "/subnetGateway/add", requestData, function(
                  data) {
                callback(data);
              });
            },
            ports : function(requestData, callback) {
              $.getJSON(one.f.switchmanager.rootUrl + "/subnetGateway/ports/add", requestData,
                  function(data) {
                    callback(data);
                  });
            }
          ...
          removeMultiple : {
            dialog : function(gatewaysToDelete) {
              ...
              var $modal = one.lib.modal.spawn(
                  one.f.switchmanager.subnetGatewayConfig.id.modal.modal, h3, $body, footer);

              // bind close button
              $('#' + one.f.switchmanager.subnetGatewayConfig.id.modal.cancel, $modal).click(
                  function() {
                    $modal.modal('hide');
                  });
      ...
      one.f.switchmanager.staticRouteConfig = {
        ...
        dashlet : function($dashlet) {
          ...
          one.f.switchmanager.staticRouteConfig.ajax.main(url, {}, function(content) {
            ...
            if (content.privilege === 'WRITE') {
              ...
              var button = one.lib.dashlet.button
                  .single("Remove Static Route",
                      one.f.switchmanager.staticRouteConfig.id.dashlet.remove, "btn-danger",
                      "btn-mini");
              var $button = one.lib.dashlet.button.button(button);
              $button.click(function() {
                var routesToDelete = [];
            ...
            $dashlet.append($gridHTML);
            var dataSource = one.f.switchmanager.staticRouteConfig.data
                .staticRouteConfigGrid(content);
        ...
        modal : {
          ...
          save : function($modal) {
            ...
            result['staticRoute'] = $(
                '#' + one.f.switchmanager.staticRouteConfig.id.modal.form.staticRoute, $modal)
                .val();
            result['nextHop'] = $(
                '#' + one.f.switchmanager.staticRouteConfig.id.modal.form.nextHop, $modal).val();
          ...
          ajax : {
            staticRouteConfig : function(requestData, callback) {
              $.getJSON(one.f.switchmanager.rootUrl + "/staticRoute/add", requestData, function(
                  data) {
                callback(data);
              });
            }
          ...
          removeMultiple : {
            dialog : function(routesToDelete) {
              ...
              var $body = one.f.switchmanager.staticRouteConfig.modal.removeMultiple
                  .body(routesToDelete);
              var $modal = one.lib.modal.spawn(
                  one.f.switchmanager.staticRouteConfig.id.modal.modal, h3, $body, footer);

              // bind close button
              $('#' + one.f.switchmanager.staticRouteConfig.id.modal.cancel, $modal).click(
                  function() {
                    $modal.modal('hide');
                  });
            ...
            var source = new StaticDataSource({
              ...
              formatter : function(items) {
                $.each(items, function(index, item) {
                  item["selector"] = '<input type="checkbox" class="staticRoute" id="'
                      + item["name"] + '"></input>';
                });
      ...
      one.f.switchmanager.spanPortConfig = {
          ...
          one.f.switchmanager.spanPortConfig.ajax.main(url, {}, function(content) {
            ...
            if (content.privilege === 'WRITE') {
              ...
              $button.click(function() {
                ...
                var checkedCheckBoxes = $(
                    "#" + one.f.switchmanager.spanPortConfig.id.dashlet.datagrid).find(
                    "tbody input:checked");
                ...
                checkedCheckBoxes.each(function(index, value) {
                  var spanPortObj = {};
                  spanPortObj['spanPortJson'] = decodeURIComponent(checkedCheckBoxes[index]
                      .getAttribute("spanPort"));
                  spanPortObj['spanPortNodeName'] = checkedCheckBoxes[index]
                      .getAttribute("spanPortNode");
                  spanPortObj['spanPortPortName'] = checkedCheckBoxes[index]
                      .getAttribute("spanPortPort");
            ...
            var $gridHTML = one.lib.dashlet.datagrid.init(
                one.f.switchmanager.spanPortConfig.id.dashlet.datagrid, {
                  searchable : true,
                  filterable : false,
                  pagination : true,
                  flexibleRowsPerPage : true
                  ...
                  $("#" + one.f.switchmanager.spanPortConfig.id.dashlet.selectAll).click(
                      function() {
                        $("#" + one.f.switchmanager.spanPortConfig.id.dashlet.datagrid).find(
                            ':checkbox').prop(
                            'checked',
                            $("#" + one.f.switchmanager.spanPortConfig.id.dashlet.selectAll).is(
                                ':checked'));
                      });
                  $(".spanPortConfig").click(
                      function(e) {
                        if (!$('.spanPortConfig[type=checkbox]:not(:checked)').length) {
                          $("#" + one.f.switchmanager.spanPortConfig.id.dashlet.selectAll).prop(
                              "checked", true);
                        } else {
                          $("#" + one.f.switchmanager.spanPortConfig.id.dashlet.selectAll).prop(
                              "checked", false);
        ...
        modal : {
          initialize : function() {
            ...
            var $modal = one.lib.modal.spawn(one.f.switchmanager.spanPortConfig.id.modal.modal, h3,
                "", footer);
            // bind save button
          ...
          save : function($modal) {
            var result = {};
            result['nodeId'] = $('#' + one.f.switchmanager.spanPortConfig.id.modal.form.nodes,
                $modal).val();
            result['spanPort'] = $('#' + one.f.switchmanager.spanPortConfig.id.modal.form.port,
                $modal).val();
            one.f.switchmanager.spanPortConfig.modal.ajax.saveSpanPortConfig(result, function(
                response) {
              if (response.status == true) {
                $modal.modal('hide');
          ...
          footer : function() {
            var footer = [];
            var saveButton = one.lib.dashlet.button.single("Save",
                one.f.switchmanager.spanPortConfig.id.modal.save, "btn-primary", "");
            var $saveButton = one.lib.dashlet.button.button(saveButton);
            footer.push($saveButton);
            return footer;
          },
          removeMultiple : {
            dialog : function(spanPortsToDelete) {
              ...
              var $body = one.f.switchmanager.spanPortConfig.modal.removeMultiple
                  .body(spanPortsToDelete);
              var $modal = one.lib.modal.spawn(one.f.switchmanager.spanPortConfig.id.modal.modal,
                  h3, $body, footer);
              ...
              // bind remove rule button
              $('#' + one.f.switchmanager.spanPortConfig.id.modal.remove, $modal).click(
                  this,
                  function(e) {
                    var requestData = {};
                    var spanPorts = [];
                    ...
                    one.f.switchmanager.spanPortConfig.ajax.main(url, requestData, function(
                        response) {
                      $modal.modal('hide');
                      if (response.status == true) {
                        // refresh dashlet by passing dashlet div as param
            ...
            footer : function() {
              ...
              var cancel = one.lib.dashlet.button.single('Cancel',
                  one.f.switchmanager.spanPortConfig.id.modal.cancel, '', '');
              var $cancel = one.lib.dashlet.button.button(cancel);
              footer.push($cancel);
              ...
              formatter : function(items) {
                $.each(items, function(index, item) {
                  item["selector"] = '<input type="checkbox" class="spanPortConfig" spanPort='
                      + encodeURIComponent(item["json"]) + ' spanPortNode="' + item["nodeName"]
                      + '" spanPortPort="' + item["spanPortName"] + '"></input>';
                });
              },
              delay : 0
            });
      ...
      one.f.connection = {
        ...
        dashlet : function($dashlet) {
          ...
          // add button
          var add = one.lib.dashlet.button.single('Add Node', one.f.connection.id.add,
