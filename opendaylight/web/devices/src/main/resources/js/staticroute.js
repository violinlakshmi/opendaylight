define(['datasource'], function(StaticDataSource) {
  ...
  var module_staticRouteConfig = {
      ...
      modal : {
        ...
        save : function($modal) {
          var result = {};
          result['routeName'] = $('#' + one.f.switchmanager.staticRouteConfig.id.modal.form.routeName,
              $modal).val();
          result['staticRoute'] = $(
          ...
          result['nextHop'] = $('#' + one.f.switchmanager.staticRouteConfig.id.modal.form.nextHop,
              $modal).val();
          one.f.switchmanager.staticRouteConfig.modal.ajax.staticRouteConfig(result,
              function(response) {
            ...
            $('#' + one.f.switchmanager.staticRouteConfig.id.modal.remove, $modal).click(
                ...
                function(e) {
                  if (routesToDelete.length > 0) {
                    ...
                    one.f.switchmanager.staticRouteConfig.ajax.main(url, requestData,
                        function(response) {
                      $modal.modal('hide');
                      if (response.status == true) {
                        // refresh dashlet by passing dashlet div as param
  ...
  return module_staticRouteConfig;
});  // end define function
