  ...
  one.main.admin = {
    ...
    body : {
      users : function(body) {
        ...
        if (one.role < 2) {
          var addUserButton = one.lib.dashlet.button.single("Add User", one.main.admin.id.add.user, "btn-primary",
              "btn-mini");
    ...
    remove : {
      ...
      footer : function() {
        var footer = [];
        var removeButton = one.lib.dashlet.button.single("Remove User", one.main.admin.id.modal.remove.user,
            "btn-danger", "");
        var $removeButton = one.lib.dashlet.button.button(removeButton);
        footer.push($removeButton);
        var modifyButton = one.lib.dashlet.button.single("Change Role", one.main.admin.id.modal.modify.user,
            "btn-success", "");
        var $modifyButton = one.lib.dashlet.button.button(modifyButton);
        footer.push($modifyButton);
        var change = one.lib.dashlet.button.single('Change Password', one.main.admin.id.modal.remove.password,
            'btn-success', '');
    ...
    add : {
      modal : {
        ...
        add : function($modal, edit, callback) {
          ...
          roles = new Array();
          roles[0] = $modal.find('#' + one.main.admin.id.modal.add.form.role).find('option:selected').attr('value');
          user['roles'] = roles;
        ...
        ajax : function(data, edit, callback) {
          if (edit) {
            $.post(one.main.admin.address.root + one.main.admin.address.modifyUser, data, function(data) {
              callback(data);
            });
          } else {
      ...
      footer : function(edit) {
        ...
        var addButton = one.lib.dashlet.button.single(buttonText, one.main.admin.id.modal.add.user, "btn-primary", "");
    ...
    password : {
      ...
      footer : function() {
        var footer = [];
        var submit = one.lib.dashlet.button
            .single('Submit', one.main.admin.id.modal.password.submit, 'btn-primary', '');
